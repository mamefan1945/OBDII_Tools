...
data class UserPreferences(
    ...
    val fuelPriceMode:        FuelPriceMode   = FuelPriceMode.MANUAL,
    val manualFuelPrice:      Float           = 1.50f,   // per litre (L100KM) or per gallon (MPG_US)
    val eiaApiKey:            String          = "",
    val eiaFuelPriceUsd:      Float?          = null,    // USD/gallon; null until first fetch
    val eiaFuelPriceUpdatedMs: Long           = 0L,
)
...
enum class FuelPriceMode(val label: String) {
    MANUAL("Manual"),
    EIA_AVERAGE("EIA Average"),
}
